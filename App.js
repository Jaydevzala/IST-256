...
app.get("/Create", function(req, res) {
 try {
    ...
    if(!req.query.CustomerName) {
		console.log("missing the CustomerName"); //Used for debugging in the console window
		//respond back with the JSON status error message
        return res.send({"result": "missing the Customer name"});
    } else {
	...
	//It is filled in with request query student field values sent
        ...
        "CustomerSSN" : req.query.CustomerSSN,
        "CustomerEmail" : req.query.CustomerEmail,
        "CustomerPhone" : req.query.CustomerPhone
        }
		console.log(Customers); //Used for debugging in the console window
        ...
        MongoClient.connect(url, function (err, client) {
          if (err) {
		    ...
		    var collection = db.collection('Customers'); //Select the Collection Students
            collection.insertOne(Customers, function (err, res){
			  if (err) throw err;
    ...
    if(!req.query.CustomerID) {
        ...
        if (err) {
          ...
          var collection = db.collection('Customers');
            collection.findOne({"CustomerID" : req.query.CustomerID}, function(err, Customer){
			  if (err) throw err;
              client.close();
...
try {
    ...
    res.header("Access-Control-Allow-Origin", "*");
    if(!req.query.CustomerName) {
        return res.send({"status": "error", "message": "missing Customer Name"});
    } else {
        var url = 'mongodb://localhost:27017';
        MongoClient.connect(url, function (err, client) {
        if (err) {
		  ...
		  var db = client.db('OakesDB');
          var collection = db.collection('Customers');
		  const query = {"CustomerID" : req.query.CustomerID};
		  console.log(query);
		  console.log(req.query.CustomerID);
		   var newvalues = { $set: {"CustomerName": req.query.CustomerName,
		   "CustomerSSN" : req.query.CustomerSSN,
           "CustomerEmail" : req.query.CustomerEmail,
           "CustomerPhone" : req.query.CustomerPhone } };
